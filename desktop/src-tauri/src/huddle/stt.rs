...
/// pre-roll buffer at every hard boundary; if that changes, re-derive their
/// overlap constraint before tuning these constants.
...
mod tests {
    ...
    fn flush_boundary_never_double_includes_audio() {
        ...
        for _ in 0..SILENCE_FLUSH_FRAMES {
            endpoint.process_frame(frame(10.0), 0.0, true, true, SILENCE_FLUSH_FRAMES);
        }
        assert!(endpoint.speech_buf.contains(&10.0));
        endpoint.reset_segment();

        for _ in 0..VAD_ONSET_FRAMES {
            endpoint.process_frame(frame(2.0), 0.9, true, true, SILENCE_FLUSH_FRAMES);
        }
        assert!(endpoint.speech_buf.iter().all(|sample| *sample == 2.0));
    }
